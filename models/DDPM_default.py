...
class LitModelDDPM(pl.LightningModule):
    ...
    def test_step(self, batch, batch_nb):
        ...
        n_images_to_sample = 4

        all_images = self.model.sample(y if y == None else y[:4], batch_size=4)  
        metrics_pred = self.model.metrics(all_images, y[:4])
        metrics_target = self.model.metrics(x[:4], y[:4])
        ...
        self.save_with_2Dlabels(all_images, mode='test', var_type='pred', n_row=2)

        if self.model_type == 'c':
            if len(y.shape) == 1:
                self.save_with_1Dlabels(y, mode='test') 
            else:
                self.save_with_2Dlabels(x, mode='test', var_type='target', n_row=2)
                self.save_with_2Dlabels(y if y.shape[1]!= 2 else y[:,0][:,None], 
                                        mode='test', var_type='input', n_row=2)
