...
#         betas = cosine_beta_schedule(self.num_timesteps)
        if state == 'train':
            ...
            betas = cosine_beta_schedule(self.num_timesteps//self.sample_every)
        alphas = 1. - betas
...
#             F.mse_loss(noise, x_recon)
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool):
        epsilon,v = self.split_diffusion_output(self.denoise_fn(x, t*self.sample_every))
        posterior_variance = (
            torch.log( self.set_min_log( extract( self.betas, t, v.shape ) ) ) * v +
            torch.log( self.set_min_log( extract( self.posterior_variance, t, v.shape ) ) ) * (1-v)
        ).exp() 
        posterior_log_variance = torch.log(self.set_min_log(posterior_variance))
        
        x_recon = self.predict_start_from_noise(x[:,:self.channels], t=t, noise=epsilon)

        if clip_denoised:
            x_recon.clamp_(-self.clip, self.clip)

        model_mean, _, _ = self.q_posterior(x_start=x_recon, x_t=x[:,:self.channels], t=t)
...
#         return model_mean + nonzero_mask * (0.5 * model_log_variance).exp() * noise
    ...
    def p_losses(self, x_start, t, y=None, noise = None):
        ...
        epsilon,v = self.split_diffusion_output(x_recon)
        
        q = self.q_posterior(x_start, x_noisy[:,:c], t)[0]
        p = self.p_sample(x_noisy, t)
        
        (q,p) = map(self.to_histo, (q,p))
        (q,p) = map(self.set_min_log, (q,p))
        
        return self.loss_simple(noise,epsilon) + self.lam*self.loss_vlb(torch.log(q),p)
