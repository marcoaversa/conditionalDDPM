...
#             F.mse_loss(noise, x_recon)
    ...
    def p_sample_loop(self, y, shape):
        ...
        for i in reversed(range(0, self.num_timesteps)):
            if self.mode == 'c':
                ...
                img = self.label_concatenate(img,self.labeldp(y))
    ...
    def energy_norm(self, image_data):
        BG=325
...
#         sigma=3.2148
    ...
    def labeldp(self, labels, t):
        for i,y in labels:
            y[i] = self.decrease_intensity(y[i],t[i].item()/self.num_timesteps)
            y[i] = self.energy_norm(y[i])
        return y
    ...
    def p_losses(self, x_start, t, y=None, noise = None):
        ...
        x_start = torch.stack([self.energy_norm(x) for x in x_start]).to(x_start.device)
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        if self.mode == 'c':
            x_noisy = self.label_concatenate(x_noisy, self.labeldp(y))
